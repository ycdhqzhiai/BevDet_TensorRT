...
// #include "bevdet/utils/cuda_util.h"
// #include "bevdet/utils/util.h"
...
template <typename T>
__global__ void resize_linear_kernel_mean_std(const unsigned char *src, float *dst,
                                          int channel, int height, int width,
                                          int stepwidth, int dst_height,
                                          int dst_width, float fx, float fy,
                                          T mean_b, T mean_g, T mean_r,
                                          bool channel_axis, float std_b, float std_g, float std_r) {
  const int dst_x = blockDim.x * blockIdx.x + threadIdx.x;
  const int dst_y = blockDim.y * blockIdx.y + threadIdx.y;
  if (dst_x < dst_width && dst_y < dst_height) {
    float src_x = (dst_x + 0.5) * fx - 0.5;
    ...
    for (int c = 0; c < channel; c++) {
      ...
      // dst[dst_idx] = (out - mean[c]) * scale;
      if (c == 0) {
        dst[dst_idx] = (out - mean_b) / std_b;
      } else if (c == 1) {
        dst[dst_idx] = (out - mean_g) / std_g;
      } else if (c == 2) {
        dst[dst_idx] = (out - mean_r) / std_r;
      }
    }
  }
}
...
bool ResizeGPU(const Image8U &src,
  ...
  resize_linear_kernel_mean_std<<<grid, block>>>(
      src.gpu_data(), dst->mutable_gpu_data() + dst->offset(start_axis),
      origin_channel, origin_height, origin_width, stepwidth, height, width, fx,
      fy, mean_b, mean_g, mean_r, channel_axis, std_b, std_g, std_r);
  return true;
}
