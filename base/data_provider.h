...
class DataProvider {
 public:
  ...
  struct ImageOptions {
    ImageOptions() {
      this->target_color = Color::NONE;
      this->do_crop = false;
    }

    ImageOptions(Color target_color, bool do_crop, RectI crop_roi) {
  ...
  // image blob with specified size should be filled, required.
  bool GetImageBlob(const ImageOptions &options, Blob<float> *blob);
#endif
  ...
  // @param [in/out]: NHWC blob (4D)
  // image blob with specified size should be filled, required.
  bool GetImageBlob(const ImageOptions &options, Blob<uint8_t> *blob);

  // @brief: get Image8U converted from raw message.
  // @param [in]: options
  // @return: Image8U
  // image blob with specified size should be filled, required.
  bool GetImage(const ImageOptions &options, Image8U *image);
 ...
 protected:
  ...
  std::shared_ptr<Image8U> ori_gray_;
  std::shared_ptr<Image8U> ori_rgb_;
  std::shared_ptr<Image8U> ori_bgr_;
